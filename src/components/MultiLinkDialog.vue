...
function onDrop(e: DragEvent): void {
  ...
  if (file && /\.txt$/i.test(file.name)) {
    ...
    reader.onerror = () => ui.toast('error', `Could not read ${file.name}`)
...
<template>
  ...
  >
    <div class="w-full max-w-lg rounded-lg border border-tx-border bg-tx-panel p-4 shadow-2xl">
      <h2 class="text-sm font-semibold">Add links</h2>
      <p class="mt-0.5 text-[11px] text-tx-muted">One link per line (spaces also work)</p>
      ...
      <div class="mt-3 flex items-center justify-between">
        <span class="text-[11px] text-tx-muted">
          {{ urls.length }} link{{ urls.length === 1 ? '' : 's' }} detected · drop a .txt file here too
        </span>
        <div class="flex items-center gap-2">
          ...
          <button type="button" class="tx-btn-accent" :disabled="!urls.length" @click="submit">
            Add {{ urls.length }} link{{ urls.length === 1 ? '' : 's' }}
          </button>
        </div>
      </div>
    </div>
