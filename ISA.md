...
- No release is published until `bun scripts/release-gate.ts` passes against the dev app: fixed end-to-end cases (YouTube above 1080p copied as VP9 with no encode, 1080p H.264 copy, subtitles, MP3/M4A/WAV/M4R, a WebM-only site, an HLS site, a silent video), every output read back with ffmpeg. A change to the finishing pass, the format selection or the picker adds its own case to the gate before it ships.
- Copy is the default for every stream. The finishing pass encodes only a codec positively known to be outside MP4 (VP8, Theora) or a codec the user forced; an unrecognised codec name is copied, never transcoded on a guess.
- Test downloads never accumulate: the release gate writes to a throwaway temp folder and deletes it when the run ends (`--keep` to inspect); any ad-hoc verification does the same. Nothing is left in Downloads, the system temp folder or the dev instance's queue after a test.
- Repository protections (2026-09-06): rulesets on GitHub, no bypass. `main`: no deletion, no force push, linear history (merges are squash or rebase; merge commits disabled, branches deleted on merge). Release tags `v*`: no deletion, no move, no force update. CI (`.github/workflows/ci.yml`) runs typecheck, unit tests and locale parity on every push and PR; Dependabot PRs must pass it. Direct pushes to main stay allowed for the release flow; the gate remains the pre-publish check.
- Published releases and their tags are permanent. GitHub's download counter lives on each release's assets (the README badge sums `downloads/total` across releases), so deleting a superseded release erases its count. Publishing marks the new release `--latest`; only unpublished drafts may be deleted. Local `release/`, `dist/`, `dist-electron/` cleanup is unaffected.
...
- 2026-09-03 14:00: Andre (Windows, 0.2.14): on the 8K HDR video the 1080p60 transfer sat at 93 % with its last speed showing and neither Pause nor Stop did anything. Without the engine.log the wedge itself is unproven; what is proven is that the app let a silent process hold a row. Two guarantees added. A kill always ends the run: children are listed before the parent dies, TERMed so aria2c saves its control file, the parent is SIGKILLed, survivors are KILLed 3 s later, and a run whose process will not exit is declared over after 5 s (`orphaned`) so the row is released; after a kill the parent's exit ends the promise rather than the pipes, so an orphan cannot hold it. A transfer that prints nothing for 90 s is killed and restarted up to twice, resuming from disk (aria2c `-c`, control file saved every 10 s); post-processing keeps the 10-minute limit. The row shows "no data for N s" after 8 s of silence instead of the last speed. Verified by SIGSTOPping aria2c: "no data for 30 s" on the row, restart logged at 90 s, progress resumed 13.5 → 20.8 %; Stop with aria2c frozen returned the row to ready in under a second with no aria2c or yt-dlp left after 4 s.

- 2026-09-03 15:10: Andre: the Mac dmg installs an app macOS calls damaged ("Move to Trash") with no Open Anyway path. Cause: `identity: null` skipped signing entirely, so a quarantined, unsigned bundle fails Gatekeeper outright on Apple silicon. Fix, matching deemix-remastered: `identity: "-"` (ad hoc) with `hardenedRuntime: true` and electron-builder's default entitlements (allow-jit, allow-unsigned-executable-memory, disable-library-validation). The bundled tools under Resources/bin and Resources/pot are excluded from signing (`signIgnore`): they carry their builders' signatures already, and electron-builder choked on PyInstaller's Python.framework inside yt-dlp ("bundle format is ambiguous"). Result: `codesign -dv` flags adhoc,runtime with identifier dev.drazy.tuberx, `codesign --verify --deep --strict` passes, spctl rejects exactly as it does Deemix, which is the state that yields "Open Anyway". Notarization needs a paid Apple developer certificate and is a separate decision.

- 2026-09-03 16:30: Andre: builds must exist in one place only. The installers had been copied into ~/Downloads after every publish (a review-artifact habit, not a build step). Rule from here: electron-builder output stays in `release/`, nothing is copied elsewhere, and `release/` is removed only on an explicit cleanup request.

- 2026-09-03 17:40: Gap closure against PullTube and WinX YouTube Downloader (assessment artifact "TuberX Gap Map", 2026-09-03). Andre's order: clipboard suggestion, reorder, subscriptions, quality picker, video formats, subtitles, trim, split, rename, logins, after-download, export, localization, plus engine-update improvement. Built and verified live in one pass, each against the running dev app: clipboard strip in the compact list state (Add / Add and download / dismiss, dedupes against the queue and remembered dismissals); drag-to-reorder with a custom dataTransfer type so URL drops are untouched, persisted to `position`; estimated finished sizes in the picker (best audio stream added to video-only rungs; MP3/WAV/M4R computed from duration and bitrate); M4R ringtone (first 40 s, AAC 128 kbps, `.m4a` renamed to `.m4r`) and Subtitles-only (`--skip-download`, output resolved from the MoveFiles line); Open file / Open with… (Windows OpenAs dialog, macOS app picker) / Copy path in the row menu, double-click opens; after-queue action (open folder, sleep, shutdown) with a 30 s cancellable banner driven by a queue `idle` event; export of queue / Later / History as text; site login (username, keychain password via safeStorage, video password, user-agent choice) gated off YouTube, argv redacted in engine.log; video codec choice (auto / H.264 / H.265) with a one-frame probe that proves each hardware encoder (VideoToolbox, NVENC, QSV, AMF) before use and an in-place conversion pass with ffmpeg progress that maps only the first video stream (the cover stream had made the encoder fail); trim with preview over a `tuberx-media://` protocol with byte-range serving, chapter presets, instant or frame-accurate MP4, M4A, M4R exports named `[trim 00m02s-00m07s]`; split by timecodes into titled numbered clips or written as chapters via an FFMETADATA file; batch rename with live preview updating rows and history; playlist/channel subscriptions (new table, launch + six-hourly check, badge count, Download new dedupes against the queue); engine self-heal (extractor-error regex, hourly throttle, dead links excluded, retries the fetch or re-queues the download); app-wide speed limit (`--limit-rate` and aria2 `--max-overall-download-limit`). Localization (eight languages, string tables under `shared/i18n`) was delegated to a worktree agent and merged after its own typecheck, key-parity script and a German live check. README rewritten as a product page with `docs/banner.png` rendered from the real icon; repository description and topics set; download, star, release and last-commit badges added.

- 2026-09-03 20:30: Andre: downloads and finishing "dog slow" against PullTube and WinX on a fast machine. Measured with the app's exact flags on a 245 MB 1080p60 video: 53 s total; extraction 3.2 s, thumbnail and subtitle fetches 6 s before any media moved, video 31 s at 7.9 MB/s, then the 10 MB audio track alone 11 s (YouTube caps every connection near 1 MB/s and yt-dlp downloads tracks one after another), merge + tags + cover under one second on this SSD. Separately, a fetch with the PO-token script provider engaged took 85 s against 2 to 3 s without it: bgutil script mode regenerates a token for every yt-dlp run and caches nothing across runs, and on a network that reaches the token endpoint (Andre's, unlike this Mac) that cost lands on every fetch and every download. aria2c refuses more than 16 connections, so more parallelism per file is not available. Decisions: (1) a parallel engine for video outputs (`electron/engine/fastpath.ts`): one offline yt-dlp run picks the formats and the final file name, then video, audio, subtitles and cover are fetched concurrently and a single ffmpeg pass merges, tags, adds chapters, subtitles and cover, with a forced codec applied in the same pass; stream files keep stable names so pause/resume and stall restarts continue from disk; any failure falls back to the classic pipeline. Result on the same video: 23 s, finishing 0.3 s. (2) PO-token helper becomes on-demand: `potHelper: 'auto' | 'always' | 'off'`, default auto, engaged only on a retry after a sign-in check, for fetch and for download. (3) every fast job ends with `fast: done in Ns: select, streams (ids, aria2|native), finish, move` in engine.log so a slow machine names its slow step. (4) `engineMode` toggle exposes the classic pipeline. Chapter titles needed `-map_metadata 0` rather than `-1` (a global reset also wipes chapter metadata).

- 2026-09-03 21:00: Andre asked for an update check against the repository. Finding: `electron-updater` was a dependency and the README promised launch-time updates, but nothing in electron/ ever called it. Built `electron/appUpdate.ts`: GitHub Releases API is the source of truth (drafts are not offered), checked 15 s after launch and every six hours while `autoCheckUpdates` is on, plus a manual check; state pushed as `update:status` to a badge in About, a dot on the Settings icon and a one-time toast. Windows installer builds download and apply in place through electron-updater; the portable exe and the ad-hoc-signed Mac build (Squirrel.Mac rejects ad-hoc signatures) open the release page instead. Verified with a faked current version 0.1.0: state available, latest 0.3.0 (the published one, the 0.3.1 draft correctly ignored), About showed "0.3.0 available" with Get the update, title-bar dot shown.

- 2026-09-05 15:50: Andre: download and star counts must not be lost while we build. Audit: the release procedure since 0.2.12 deleted the previous release and tag after each publish (0.2.12 → 0.3.0, seven releases), and GitHub keeps `download_count` per asset per release, so every count before 0.3.1 is gone; the README badge only ever saw the one surviving release. Stars are repository-level and were never affected (0 today). Rule added to Constraints: releases and tags are permanent, drafts only may be deleted. Counts at the time of the rule: v0.3.1 assets 15 downloads total (installers: Portable 3, Setup 2, dmg 1, zip 1), 140 clones from 58 sources in 14 days.

- 2026-09-05 16:10: Andre asked whether TuberX downloads from every site on yt-dlp's supported-sites list. Evidence gathered rather than asserted. (1) Extractor parity: the bundled engine is 2026.08.19, the current upstream release; `--list-extractors` names match supportedsites.md exactly (1751 extractors, 136 of them flagged CURRENTLY BROKEN by upstream itself; the only differences are zero-width-space formatting and upstream's hidden internal pseudo-extractors such as DRM, Piracy, TestURL). The app applies no site filter: any http(s) URL goes to yt-dlp. (2) Live sweep through the dev app over CDP, one yt-dlp test URL per site for 30 sites, lowest rung, temp destination: 17 downloaded (YouTube, Dailymotion, SoundCloud, Reddit, Bilibili, Twitch clip, Streamable, VK, Mixcloud, Loom, Dropbox, NYTimes, Pinterest, Substack, iHeart, Instagram, Nebula). 13 failed: 2 defects in TuberX (9gag: VP8 source, the finishing pass attached the cover to a WebM container which forbids MJPEG, then the classic remux to MP4 failed because VP8 cannot be remuxed into MP4; tumblr: audio output from a video source, mutagen failed silently under --no-warnings, no AtomicParsley, and yt-dlp's ffmpeg fallback needs ffprobe, which is not shipped), 2 test artefacts (archive.org Cops 1922 and the Imgur clip are silent videos and the driver asked for audio; the picker should not offer audio when the source has no audio stream), 2 upstream extractor breaks in 2026.08.19 (TED, Facebook), 7 site-side (Vimeo DRM on that film, Bandcamp 403, Rumble 403, TikTok IP block, Twitter domain error, PBS and Audiomack test URLs dead). Fix plan, pending Andre: bring audio outputs onto the parallel engine (single ffmpeg pass for extraction, tags, cover, chapters; no dependency on yt-dlp post-processors or ffprobe), and make the finishing pass transcode VP8/VP9/Vorbis sources to H.264/AAC when the target is MP4 and never attach a cover to WebM. Sweep files, dev queue rows and the dev app were removed afterwards.

- 2026-09-05 17:30: Andre: fix both sweep defects and ship 0.3.2. (1) Audio outputs join the parallel engine (`fastpath.ts`): one offline selection run picks the best audio stream (a video+audio pair resolves to its audio half; a muxed single file is downloaded whole), stream and cover arrive together, and one ffmpeg pass extracts or encodes (M4A copies AAC/ALAC and otherwise encodes AAC between 128 and 256 kbps from the source rate; MP3 at the configured bitrate; WAV 16-bit PCM; M4R AAC 128 kbps cut at 40 s inside an M4A container renamed .m4r), writes tags and chapters and embeds the cover (attached_pic for M4A/M4R, APIC via ID3v2.3 for MP3, none for WAV). yt-dlp's ExtractAudio, Metadata and EmbedThumbnail post-processors are no longer on the path, so ffprobe, mutagen and AtomicParsley are not needed; the classic pipeline remains the fallback. (2) The finishing pass now decides the container and codecs: a single WebM/MKV source becomes MP4 when "convert to MP4" is on (otherwise the classic pipeline keeps the container), video codecs MP4 cannot carry (VP8, VP9) are encoded to H.264 with the best available encoder, audio codecs it cannot carry (Vorbis, Opus) become AAC 160 kbps, and cover art is only ever attached to MP4. (3) Picker: a format with a height but no reported video codec counts as video (archive.org, generic pages), and audio options are hidden only when every format says acodec "none". Verified live over CDP against the dev app with ffmpeg reading every output: 9gag VP8+Vorbis → H.264/AAC MP4 with cover in 2.7 s (was: finishing failed, classic remux failed); tumblr and Dailymotion video → M4A with cover (was: EmbedThumbnail "ffprobe not found"); SoundCloud M4A copied (AAC 160k, cover); YouTube Opus → MP3 320k with chapters and cover, → WAV PCM with tags, → M4R 40.00 s AAC with cover; 9gag VP9 webm → MP3; archive.org Cops (1922) and an Imgur clip now default to v:best and download as silent MP4s (were: audio offered on a silent source); YouTube video with mov_text subtitles and a 360p pick unchanged.

- 2026-09-05 18:20: Andre, on 0.3.2: every download "is doing this conversion" and finishing is far slower. His engine.log: format 315 (YouTube 2160p60) → `codec vp9 → h264 with h264_videotoolbox in the finishing pass`. Cause: the 0.3.2 rule "encode what MP4 cannot carry" used an allow-list that omitted VP9 and AV1, and YouTube serves everything above 1080p and all HDR only as VP9 (or AV1); 0.3.1 copied those streams into MP4 untouched, so every 4K/8K download in 0.3.2 gained a full H.264 transcode. Why it shipped: the 0.3.2 verification ran twelve cases, all at 1080p or below and all H.264 sources; the copy decision for high resolutions was never exercised. Andre: "stop introducing regressions … regression is not an option". Fix (0.3.3): VP9, AV1 and MJPEG join the MP4 list, an unrecognised codec is copied rather than encoded, unknown audio is copied, and the temp file keeps the final container's extension. Verified on synthetic VP9/Opus and VP8 WebMs (VP9 → copy, Opus → AAC, VP8 → encode; VP9 muxed into MP4 as vp09) and through the release gate. Structural change: `scripts/release-gate.ts`, twelve fixed end-to-end cases through the dev app with ffmpeg assertions on codec, cover, subtitles, duration, container and on whether the finishing pass encoded; a YouTube case above 1080p asserts "never encoded, finish ≤ 5 s". Rule added to Constraints: no publish without a passing gate, and copy is the default for every stream.

- 2026-09-05 18:50: Andre, before 0.3.3 ships: users must be told in the app when a new build exists, at launch and on a schedule they can choose; he doubted the earlier implementation. Audit of 0.3.1/0.3.2: the check existed (15 s after launch, then every six hours, a badge in About, a dot on Settings, a plain four-second toast without an action, no log line), so a user whose app was already open when a release landed waited up to six hours, and a missed toast left only the dot. Changes: `updateCheckEvery` setting ('hour' default, 'sixHours', 'day', 'launch', 'never'; the old boolean migrates at settingsVersion 14, off → never); the schedule is re-applied the moment the setting changes; a check also runs when the machine wakes from sleep past the interval; the new-version toast now carries a "Get the update" button and stays until dismissed or acted on (Windows installer: downloads and applies in place; portable and Mac: opens the release page); every check and every schedule change is written to engine.log as `[update] …`; README and all eight locales updated. Verified live with TUBERX_FAKE_VERSION=0.1.0: toast "TuberX 0.3.2 is available / Get the update" 16 s after launch and still present after the About panel was opened, Settings dot on, status available, About select with the five options, schedule change day → hour logged within the same second.

- 2026-09-05 19:20: The release gate's SoundCloud case failed with "download finished but no output file was reported" although the file was on disk. Trail: the saved stream URL had expired (SoundCloud signs its HLS URLs for minutes, the app trusted saved metadata for three hours), the fast path got a 403 and fell back, the classic run got the same 403, refetched as designed, wrote the file and printed TXOUT, and the result was thrown away: both classic retries (PO-token retry and stale-metadata refetch) ran with a log-only line handler, so TXOUT, progress and "already downloaded" were never read on a retry. Present since those retries were added (0.2.x), so every user who queued links and downloaded later could see a finished file reported as failed. Fix: one `launch(argv)` helper carries the full line parser for every attempt; the fast path itself now refetches metadata once on a 403/expired stream and retries before falling back. Separately confirmed with the bundled yt-dlp from the command line that YouTube is refusing anonymous requests from this IP ("Sign in to confirm you're not a bot") regardless of cache or IPv4 after the day's test volume, and this network sinkholes the PO-token endpoint; the gate's seven YouTube cases cannot run from here until that lifts. Rule holds: no publish without them.

- 2026-09-06 09:40: Andre: an ultra-wide video (youtu.be/icyft1dDr6g, 2.39:1) offered only "1440p" although YouTube shows 4K. Format list: 401/313 are 3840×1608 (YouTube note "2160p"), 271/400 are 2560×1072 ("1440p"), 137 is 1920×804 ("1080p"). The picker grouped rungs by `snapHeight(height)`, so 1608 snapped to 1440 and the true 4K stream carried a 1440p label; a "1440p" pick sorted with `res:1440` and landed on the 1072-tall stream. Fix: `rungOf(format)` takes YouTube's `format_note` rung when present, otherwise the rung a 16:9 frame with the same long side would have (`snapHeight(max(short, long·9/16))`), which also keeps portrait Shorts at 1080p; each rung's `-S res:` value is now its real smallest dimension (1608, 1072, 804), so a pick lands on that rung and not a neighbour. Tests added for ultra-wide, portrait and plain frames. Two gate cases added for this video: 4K rung must produce a 3840-wide copy, 1080p rung a 1920-wide copy.

- 2026-09-06 10:20: Gate on the rung fix: 13/14, the miss a 9gag fetch reported as "unknown error" while the site answered a direct probe at once; the case passed on an immediate re-run through the app (2 s, VP8 → H.264). "unknown error" was the translation of an empty stderr, so a killed or crashed fetch had no trace anywhere. Changes: a fetch that fails without a message now says so ("yt-dlp exit N, no message") and every failed fetch is written to engine.log under `[fetch]`; the log helper moved to `electron/engine/log.ts` (re-exported from the queue manager) so the engine can log without importing the manager.

- 2026-09-06 12:30: Andre: clean up the Security tab and take the Dependabot PRs. Findings: the two secret-scanning alerts were Google's public YouTube client API keys inside a Deno compile cache committed and removed on 2026-09-02 (6,848 and 286 public copies on GitHub); resolved as false positives with the reasoning on each. Dependabot, security updates, private vulnerability reporting and CodeQL default setup enabled; SECURITY.md and a grouped dependabot.yml added. Dependabot then listed 18 advisories, all Electron (we shipped 38.8.6; fixes in 39.8.x, and 39 itself reached end of life on 2026-05-05), and opened three PRs: Electron 44.1.1, the app group (electron-store 11, pinia 4: ESM-only, no functional change), the tooling group (Tailwind 4, TypeScript 7, Vite 8, vite-plugin-electron 1: three breaking majors, no security content). Staged: Electron first, to the current major 44.2.0 (Chromium 152, Node 24.20, supported to 2027-03). Two breaks found and fixed: `clipboard.readText()` is asynchronous (typecheck caught the two call sites); the trim preview's `tuberx-media://` scheme was refused by the <video> element (MEDIA_ERR_SRC_NOT_SUPPORTED, empty message) and by fetch, which the gate does not cover and typecheck cannot see. Chromium 152 loads media only from a `standard` scheme, and Electron 44's advisory fix refuses cross-origin loads of a fetch-capable scheme without `corsEnabled`; both privileges added and CORS headers returned. Verified: gate 14/14 on Electron 44 (before the media fix), trim dialog readyState 4 and seek to 7 s after it, async paste and context menu working, tools all detected. Electron's breaking-changes list 39→44 was read entry by entry: macOS 12 dropped (every Apple-silicon Mac runs 13+), 32-bit Windows dropped (x64 only shipped), macOS notifications on the UNNotification API (checked in the packaged build below). The tooling and app groups follow as their own gated stages, or are declined with the reason on the PR.

- 2026-09-06 13:40: Packaged smoke on Electron 44 hit the single-instance lock: Andre's installed TuberX was running, so the release build quit at once (the app was left alone). `TUBERX_USER_DATA` now overrides the data folder (testing only, documented in main.ts) so a packaged build can be exercised beside an installed one. Electron 44 moved macOS notifications to the UNNotification API; the main process cannot be reached over the inspector for a direct check, so the completion notification now writes `[notify] shown:` or `[notify] failed:` to engine.log, and the packaged smoke exercises the real path: one download with notifications on, then the log line. Same line lets a user prove the feature on their machine.

- 2026-09-06 15:30: Andre asked for selecting and removing entries in the download list and History, singly and all at once, and reported the trim dialog on an 8K HDR 120 fps video (Masterpiece OLED, 61 min) "loads but nothing happens". Inventory: the queue had click/Cmd-click/Cmd+A selection and a Delete-key removal that never fired on a Mac (the Mac delete key reports "Backspace"); the right-click "Remove from list" ignored the selection; nothing visible announced a selection; History and Later had only per-entry × and "Clear history". Built (0.3.6): Delete and Backspace remove the selection; a "Remove selected (N)" footer button while anything is selected and "Clear finished (N)" otherwise; the row menu removes the whole selection when the clicked row is part of one; History and Download Later get the same selection model through a shared `useSelection` composable (click, Cmd-click, Cmd+A routed to the open drawer, Delete/Backspace, "Remove selected (N)" in the header, "Clear history" kept). Trim: the dialog no longer depends on the <video> element decoding the file. `media.info` (ffmpeg) supplies the duration so Set in/out, chapter presets and Export are usable at once; a preview that errors, never loads within 8 s, or plays without producing a frame shows "The preview can't play this file here…" while everything else works; each open is logged as `[trim] open: <file> (vcodec/acodec, duration)`. On this Mac 4K HDR VP9 previews decode fine (174 frames in 3 s), and Electron's Chromium decoded MPEG-2 and MPEG-4 Part 2 test files too, so the unavailable path was proven with an unreadable file (video error 4 → notice shown). Andre's Windows case is still to be confirmed from his engine.log `[trim]` line. Live-verified: footer "Remove selected (2)" and Backspace removal; "Clear finished (1)/(2)" click; History Cmd+A → "Remove selected (2)", Cmd-click deselect → (1), Backspace removed only history entries (queue untouched); Later Cmd+A + Backspace → 0; trim buttons enabled before metadata, export 2–7 s on a non-YouTube codec file.
...
- ISC-55: silent transfer restarts itself. aria2c SIGSTOPped at 13.5 %: row read "no data for 30 s · 33.0 MB / 245.0 MB · file 1/2 · aria2"; engine.log "watchdog: no data for 90 s; restarting the transfer (1/2)"; new aria2c resumed, 20.8 % fifteen seconds later (2026-09-03 13:55).
- ISC-56: Stop lands while the downloader is wedged. aria2c SIGSTOPped, cancel → status ready in 0 s; no aria2c or yt-dlp process 4 s later; temp and destination empty (2026-09-03 13:59).
- ISC-57: Mac bundle ad-hoc signed. `codesign -dv release/mac-arm64/TuberX.app` → `Identifier=dev.drazy.tuberx`, `flags=0x10002(adhoc,runtime)`; `codesign --verify --deep --strict` → valid on disk, satisfies its Designated Requirement; entitlements allow-jit, allow-unsigned-executable-memory, disable-library-validation; spctl rejected for both TuberX and Deemix (parity, 2026-09-03 15:12).
- ISC-58: clipboard strip: link copied → focus → strip shown → "Add and download" → row added and auto-started, strip cleared (CDP, 2026-09-03 15:00).
- ISC-59: reorder: `reorderRows([b,a])` persisted and reloaded in that order; rows carry `draggable` (2026-09-03 15:05).
- ISC-60: picker sizes: `v:best 617KB | v:240 617KB | v:144 493KB | a:m4a 302KB` for jNQXAC9IVRw; ringtone `Me at the zoo.m4r` 19 s AAC 127 kbps; subtitles-only produced `Me at the zoo.en.srt` and the row's outputPath points at it (2026-09-03 15:10).
- ISC-61: after-queue sleep: banner "Sleeping in 28 s — the queue is finished." after the last row finished; Cancel removed it; machine stayed awake (2026-09-03 15:14).
- ISC-62: logins: `secrets.json` holds the password encrypted (no plaintext); YouTube download argv carries the iPhone user-agent and `--video-password ••••` but no `--username`; a Vimeo fetch with the stored login failed with "Unable to log in: bad username or password", proving the credentials reached the site (2026-09-03 15:18).
- ISC-63: codec conversion: `tools.encoders()` → VideoToolbox for both codecs; H.265 download → `Video: hevc (Main) (hvc1)` with artist tag, three chapters and cover intact; engine.log "converted to h265 with Apple VideoToolbox" (2026-09-03 15:23).
- ISC-64: trim: `<video>` over `tuberx-media://` reached readyState 4, duration 19.1, seek to 10 s worked; exports 2–7.5 s instant and precise both 5.53 s h264/aac; ringtone from 1–60 s capped to the 18 s available (2026-09-03 15:30).
- ISC-65: split: three clips 5.02 / 12.02 / 2.06 s with titles Intro / The cool thing / End; chapters written into a copy (0–9 "One", 9–19.06 "Two"); rename moved the file, row outputPath and one history row followed; both dialogs render (2026-09-03 15:36).
- ISC-66: subscriptions: @jawed/videos → total 1, 0 new; known cleared → check found 1 new; Download new deduped against the existing row and cleared the new list; panel and title-bar badge render (2026-09-03 15:44).
- ISC-67: self-heal: unsupported link logged "self-heal: extractor error, checking for a newer yt-dlp" then "yt-dlp 2026.08.19 is already current"; dead-link filter added afterwards. Speed limit 300 KB/s: argv `--limit-rate 300K` and aria2 `--max-overall-download-limit=300K`, observed 269 KB/s (2026-09-03 15:50).
- ISC-68: localization merged: typecheck clean, 26 unit tests pass, i18n-check 391/391 for all seven locales; live Japanese footer "3 件 | すべてに適用… | 最高画質 …", German title-bar labels "Später herunterladen, Verlauf, Abonnements, Einstellungen", back to en on auto (2026-09-03 18:05).
- ISC-69: regression caught by the README screenshot and fixed: the compact clipboard strip had captured the empty state's v-else, so a populated list showed the empty state; after wrapping the compact branches in one template, 2 rows rendered and no empty-state heading was present (2026-09-03 18:12).
- ISC-70: parallel engine on aqz-KE-bpKQ 1080p60: `fast: done in 23.1s: select 0.4s, streams 22.3s (299+140, aria2), finish 0.3s, move 0s` against 53.0 s for the classic pipeline with identical flags; output h264 + aac + mjpeg cover, title/artist/date tags (2026-09-03 20:06).
- ISC-71: fast path correctness on jNQXAC9IVRw: three chapters with titles, eng mov_text subtitle track, cover, tags; keep-both produced `Me at the zoo [144p].mp4` beside the plain file; pause at 35 % left `.part`/`.aria2` files, resume continued to done in 4.9 s, temp folder empty afterwards (2026-09-03 20:12).
- ISC-72: PO helper cost: fetch 2.1–3.0 s warm without the helper, 84.7 s with the script provider on this network; default changed to on-demand (2026-09-03 19:40).
- ISC-73: update check: `update.check()` with TUBERX_FAKE_VERSION=0.1.0 → `available / 0.3.0 / releases/tag/v0.3.0`; About badge and button rendered; title-bar dot present; canInstall false in dev (2026-09-03 21:05).
- ISC-74: audio on the parallel engine, all four kinds, ffmpeg-read outputs: MP3 320k + APIC + 3 chapters; M4A aac copy + attached_pic; WAV pcm_s16le + tags; M4R 40.00 s aac 127k + attached_pic; video→M4A (Dailymotion, tumblr) aac + attached_pic; `fast: audio opus → mp3`, finish 0.1–0.9 s (2026-09-05 17:10).
- ISC-75: WebM-only source to MP4: 9gag 460sv-vp8 → `codec vp8 → h264 with h264_videotoolbox (MP4 cannot carry the source codec)`, output h264/aac/mjpeg attached_pic, 2.7 s total; classic remux no longer reached (2026-09-05 17:05).
- ISC-76: silent sources: archive.org Cops1922 options `v:best v:480 v:360 vo:best …`, default v:best, 156 MB h264 MP4 with cover and no audio stream; Imgur A61SaA1 likewise (2026-09-05 17:12).
- ISC-77: update notification: with a faked 0.1.0, `[update] check (scheduled): 0.3.2 available` at +16 s; toast text "TuberX 0.3.2 is available Get the update", hasButton true, settings dot true, toast persisted; About select present with five options; `settings.set({updateCheckEvery:'day'})` → `[update] schedule: day (every 1440 min)` (2026-09-05 18:38).
- ISC-78: release gate 12/12 on the 0.3.3 code (2026-09-05 19:35): YouTube 1440p vp9/aac copy finish 0.3 s (no encode), 1080p60 h264 copy, subtitles mov_text, MP3/M4A/WAV/M4R, SoundCloud after the stale-URL fix, Dailymotion HLS, 9gag VP8 encoded 0.8 s, archive.org silent film; `bun scripts/release-gate.ts` exit 0.
- ISC-79: ultra-wide rungs: gate cases `YouTube ultra-wide 4K rung` → vp9 3840x1608/aac copy finish 0.3 s and `YouTube ultra-wide 1080p rung` → h264 1920x804/aac copy finish 0.2 s; full gate 13/14 with the 9gag transient re-run passing (2026-09-06 10:15).
