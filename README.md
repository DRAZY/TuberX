...
- **Pause, Stop, Resume, Download again.** Pause keeps what has been transferred and Resume continues from there (aria2c and yt-dlp both resume partial files). Stop discards the partial and returns the row to Ready with the quality dropdown live, so you can pick a different format and download again. Finished rows have a Download-again control for the same purpose. Downloading a different video quality keeps the earlier file and saves the new one with the quality in its name ("Title [1080p].mp4"); downloading the same format again refreshes that file in place; audio formats have their own extensions and never collide. Settings › Downloads can switch this to "Replace the existing file".
...
- **Downloads:** re-download naming (keep both files or replace), skip files already downloaded, concurrent downloads (1–8), aria2c on/off, notifications.
